...
def outputMetar1(disp, station, condition):
    ...
    draw1.text((x, top + 0), station[0][1:] + "-" +
               condition["flightCategory"], font=fontLarge, fill=255)
    w, h = fontXSmall.getsize(str(station[1]))
    draw1.text((width-w, top + 1), str(station[1]), font=fontXSmall, fill=255)

    draw1.text((x, top + 20), condition["windDir"] + "@" + str(condition["windSpeed"]) + (("G" + str(
        condition["windGustSpeed"]) if condition["windGust"] else "") + "/" + str(condition["vis"]) + "SM "), font=fontMed, fill=255)
    draw1.text((x, top + 35), str(condition["altimHg"]) + "Hg" + " " + str(
        condition["tempC"]) + "/" + str(condition["dewpointC"]) + "C", font=fontMed, fill=255)
    draw1.text((x, top + 50), condition["obsTime"].astimezone(central).strftime(
        "%H:%MC") + " " + condition["obsTime"].strftime("%H:%MZ"), font=fontSmall, fill=255)
    disp.image(image1)
    disp.show()
...
def outputMetar2(disp, station, condition):
    ...
    draw2.rectangle((0, 0, width, height), outline=0, fill=0)
    draw2.text((x, top + 0), station[0][1:] + "-" +
               condition["flightCategory"], font=fontLarge, fill=255)
    w, h = fontXSmall.getsize(str(station[1]))
    draw2.text((width-w, top + 1), str(station[1]), font=fontXSmall, fill=255)

    yOff = 18
    xOff = 0
    NewLine = False
    for skyIter in condition["skyConditions"]:
        draw2.text((x + xOff, top + yOff), skyIter["cover"] + ("@" + str(
            skyIter["cloudBaseFt"]) if skyIter["cloudBaseFt"] > 0 else ""), font=fontSmall, fill=255)
        if NewLine:
            yOff += 12
            xOff = 0
            NewLine = False
