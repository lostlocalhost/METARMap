...
pixels = neopixel.NeoPixel(LED_PIN, LED_COUNT, brightness=LED_BRIGHTNESS_DIM if (
    ACTIVATE_DAYTIME_DIMMING and bright == False) else LED_BRIGHTNESS, pixel_order=LED_ORDER, auto_write=False)
...
url = "https://www.aviationweather.gov/adds/dataserver_current/httpparam?dataSource=metars&requestType=retrieve&format=xml&hoursBeforeNow=5&mostRecentForEachStation=true&stationString=" + \
    ",".join([item for item in list(airport_dict.keys()) if "NULL" not in item])
print(url)
    ...
    if disp is not None:
        if displayTime <= DISPLAY_ROTATION_SPEED:
            if displayTime <= DISPLAY_ROTATION_SPEED/2:
                displaymetar.outputMetar1(disp, displayList[displayAirportCounter], conditionDict.get(
                    displayList[displayAirportCounter][0], None))
            else:
                displaymetar.outputMetar2(disp, displayList[displayAirportCounter], conditionDict.get(
                    displayList[displayAirportCounter][0], None))
